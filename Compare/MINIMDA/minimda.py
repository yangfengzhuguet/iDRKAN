...
class MINIMDA(nn.Module):
    ...
    def forward(self, mm_graph, dd_graph, md_graph, miRNA, disease, samples):
        ...
        mi_index = samples[:,0]
        mi_index = torch.tensor(mi_index).int()
        di_index = samples[:,1]
        di_index = torch.tensor(di_index).int()
        emb_m = torch.index_select(emb_mm, 0, mi_index)
        emb_d = torch.index_select(emb_dd, 0, di_index)
        emb = emb_m * emb_d
        # emb = th.cat((emb_mm[samples[:, 0]], emb_dd[samples[:, 1]]), dim=1)
        return torch.sigmoid(self.mlp_(emb)).view(-1)